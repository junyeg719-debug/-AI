...
const BASIC_FIELDS: AnyField[] = [
  { key: 'height',    label: '身長',   type: 'number', unit: 'cm', min: 140, max: 180 },
  { key: 'bodyType',  label: '体型',   type: 'select', options: ['スリム','普通','グラマー','ぽっちゃり','やや細め'] },
...
const DETAIL_FIELDS: AnyField[] = [
  { key: 'nickname',      label: 'ニックネーム',      type: 'text',        placeholder: '例：まい' },
  ...
  { key: 'jobName',       label: '職業名',            type: 'text',        placeholder: '例：OL' },
...
const INITIAL_VALUES: Record<string, string | string[]> = {
  height: '158', bodyType: 'スリム', bloodType: '', location: '大阪', birthplace: '',
  jobType: '会社員（非上場）', education: '大学卒', income: '200〜400万円未満', smoking: '吸わない',
  nickname: 'まい', siblings: '長女', language: ['日本語'], school: '', jobName: '',
  marriage: '独身（未婚）', children: 'なし', marriageIntent: '2〜3年のうちに',
  wantChildren: '子供は欲しい', housework: '協力したい', meetingHope: '気が合えば会いたい',
  dateCost: 'ワリカン', mbti: '', personality: ['明るい', '優しい'], sociability: 'すぐ打ち解ける',
  roommate: '一人暮らし', pet: [], holiday: '土日', alcohol: 'ときどき飲む', hobbies: ['カフェ巡り', '旅行'],
}

const DEFAULT_BIO = 'はじめまして！\nプロフィールを見ていただき、ありがとうございます😊\n\n大阪でOLをしています。土日休みです。\n\n休日はカフェ巡りや旅行が大好きで、いつも新しいお店を開拓しています☕\n\n笑顔が素敵な方と一緒に楽しい時間を過ごしたいと思っています✨\n\nよろしくお願いします！'
        ...
        <div className="overflow-y-auto flex-1 px-4 py-3">
          ...
          {field.type === 'number' && (
            <div className="space-y-0.5 py-2">
              {Array.from({ length: (field.max ?? 180) - (field.min ?? 140) + 1 }, (_, i) => String((field.min ?? 140) + i)).map(opt => (
...
function compressImage(base64: string, maxPx = 800, quality = 0.72): Promise<string> {
...
export default function FemaleProfileEditPage() {
  ...
  useEffect(() => {
    const savedProfile = getStore<Record<string, string | string[]> | null>(STORAGE_KEYS.profile, null)
    if (savedProfile) setValues(prev => ({ ...prev, ...savedProfile }))
    const savedBio = getStore<string | null>(STORAGE_KEYS.bio, null)
    if (savedBio) setBio(savedBio)
    const savedPhotos = getStore<(string | null)[] | null>(STORAGE_KEYS.photos, null)
    if (savedPhotos && savedPhotos.some(Boolean)) setPhotos(savedPhotos)
    setIsLoaded(true)
  }, [])
  ...
  const handlePhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setPhotos(prev => prev.map((p, i) => i === activePhotoIdx ? compressed : p))
  }
  ...
  const saveField = (key: string, val: string | string[]) => {
    setValues(prev => {
      ...
      setStore(STORAGE_KEYS.profile, next)
      return next
    })
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white px-4 pt-10 pb-3 sticky top-0 z-20 shadow-sm flex items-center justify-between">
        <button onClick={() => router.push('/demo-female/profile')}
      ...
      <div className="bg-white px-4 pt-5 pb-5 mb-3">
        ...
        <div className="flex gap-2 mb-2">
          <button onClick={() => openPhotoPicker(0)}
            ...
            {photos[0] && <img src={photos[0]} alt="メイン" className="w-full h-full object-cover" />}
          ...
          <div className="flex-1 flex flex-col gap-2">
            {[1, 2].map(idx => (
              <button key={idx} onClick={() => openPhotoPicker(idx)}
                ...
                {photos[idx] && <img src={photos[idx]!} alt={PHOTO_LABELS[idx]} className="w-full h-full object-cover absolute inset-0" />}
                <div className="absolute top-2 right-2 w-7 h-7 bg-white rounded-full shadow flex items-center justify-center">
                  <Pencil className="w-3.5 h-3.5 text-gray-600" />
                </div>
              </button>
