...
function getAiSuggestions(partnerName: string, partnerInterests: string[], lastPartnerMsg: string): string[] {
  ...
  // If partner said something, generate a contextual response
  if (lastPartnerMsg.includes('コーヒー') || lastPartnerMsg.includes('カフェ')) {
    return [
      `コーヒー好きなんですね！私もよくカフェに行きます☕ おすすめのお店はありますか？`,
      `ラテアートって見てるだけで癒されますよね🎨 どんなデザインが得意ですか？`,
      base[2],
    ]
  }
  if (lastPartnerMsg.includes('花') || lastPartnerMsg.includes('フラワー')) {
    return [
      `お花に囲まれたお仕事、素敵ですね🌸 どんな種類のお花が一番好きですか？`,
      `フラワーアレンジメントとかも作るんですか？センスありそうで気になります✨`,
      base[4],
    ]
  }
  if (lastPartnerMsg.includes('動物') || lastPartnerMsg.includes('ハリネズミ')) {
    return [
      `動物カフェって入ったことないんですが、ハリネズミカフェってどんな感じですか😊`,
      `看護師さんなんですね！お仕事お疲れ様です。休みの日に動物カフェでリフレッシュしてるんですね🦔`,
      `${interest2}も好きなんですか？一緒に行けたら楽しそうですね！`,
    ]
  }
...
export default function DemoChatPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lastPartnerMsg =
    messages.filter((m) => m.sender_id !== DEMO_USER_ID).at(-1)?.content ?? ''

  const aiSuggestions = partner
    ? getAiSuggestions(partner.name, partner.interests, lastPartnerMsg)
    : []
  ...
  return (
    <div className="flex flex-col h-screen" style={{ background: '#F8F5F6' }}>
      ...
      <div className="bg-white px-4 pt-10 pb-3 shadow-sm flex items-center gap-3 sticky top-0 z-10">
        ...
        <div
          className={`w-10 h-10 rounded-full bg-gradient-to-br ${partner.color} flex items-center justify-center text-xl flex-shrink-0 relative`}
        >
          {partner.emoji}
          {partner.isOnline && (
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-gray-400">
            {partner.age}歳 ・{partner.location}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center py-8 gap-3">
            <div
              className={`w-20 h-20 rounded-full bg-gradient-to-br ${partner.color} flex items-center justify-center text-4xl shadow-md`}
            >
              {partner.emoji}
            </div>
              ...
              {msgs.map((msg) => {
                ...
                return (
                  ...
                  >
                    {!isMine && (
                      <div
                        className={`w-7 h-7 rounded-full bg-gradient-to-br ${partner.color} flex items-center justify-center flex-shrink-0 text-sm`}
                      >
                        {partner.emoji}
                      </div>
                    )}
                    ...
                    >
                      <div
                        className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${
                          isMine ? 'rounded-br-sm text-white' : 'bg-white text-gray-800 shadow-sm rounded-bl-sm'
                        }`}
                        style={
                          isMine
                            ? { background: 'linear-gradient(135deg, #7E2841, #A03558)' }
                            : undefined
                        }
                      >
                        {msg.content}
                      ...
                      <span className="text-xs text-gray-400 px-1">
                        ...
                        {isMine && (
                          <span className="ml-1" style={{ color: '#7E2841' }}>
                            既読
                          </span>
                        )}
                      </span>
        ...
        {partnerTyping && (
          <div className="flex items-end gap-2">
            <div
              className={`w-7 h-7 rounded-full bg-gradient-to-br ${partner.color} flex items-center justify-center text-sm`}
            >
              {partner.emoji}
            </div>
        ...
        <div className="bg-white border-t border-gray-100 px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-sm font-bold" style={{ color: '#7E2841' }}>
                AI返信サポート
              </p>
            </div>
            <button
          ...
          <div className="space-y-2">
            {aiSuggestions.map((suggestion, i) => (
              <button
                ...
                className="w-full text-left text-sm px-3 py-2.5 rounded-xl border transition active:scale-[0.99]"
                style={{
                  borderColor: '#E8E0E2',
                  background: '#FAFAFA',
                  color: '#1A1A1A',
      ...
      <div className="bg-white border-t border-gray-100 px-4 py-3 pb-6">
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            style={{
              background: showAiPanel ? '#7E2841' : '#F5E6EA',
              color: showAiPanel ? '#fff' : '#7E2841',
            }}
          >
